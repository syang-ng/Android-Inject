...
int inject_remote_process(pid_t target_pid, const char *library_path, const char *function_name,
    ...
    LOGI("hello world %x %x %x", library_path, function_name, param);
...
Java_cn_syang2forever_androidinject_MainActivity_injectProcess2(JNIEnv *env, jobject instance,
    ...
    const char *fun = (*env)->GetStringUTFChars(env, fun_, 0);
    const char *param = (*env)->GetStringUTFChars(env, param_, 0);

    int ret =  inject_remote_process(pid, so, fun, param, strlen(param));

    (*env)->ReleaseStringUTFChars(env, so_, so);
    (*env)->ReleaseStringUTFChars(env, fun_, fun);
    (*env)->ReleaseStringUTFChars(env, param_, param);
    return ret;
}